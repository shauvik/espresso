...
import static android.support.test.espresso.web.matcher.DomMatchers.withBody;
...
public class DomMatchersTest extends TestCase {
  ...
  public void testWithBody_NormalDocument() {
    Matcher<Element> hasAttributes = new TypeSafeMatcher<Element>() {
      @Override
  ...
  public void testWithBody_DocumentWithNoBody() throws Exception {
    Document documentWithNoBody = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(
        new StringBufferInputStream(HTML_NO_BODY));
  ...
  public void testWithXPath() {
    assertTrue(elementByXPath("/html/body/button", withTextContent("click here"))
        .matches(document));
    assertTrue(elementByXPath("/html/body/span[3]", withTextContent("Test"))
        .matches(document));
    assertFalse(elementByXPath("/html/body/some_bogus_path", withTextContent("click here"))
        .matches(document));
  }

  public void testWithMultipleMatchesForXPath() {
    try {
      assertTrue(elementByXPath("/html/body/span", withTextContent("click here"))
          .matches(document));
